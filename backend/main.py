...
def get_archive_filename():
    ...
    return DATA_DIR / f"cache.{today}.json"
...
def save_to_cache(data: dict, from_force: bool = False):
    ...
    print(f"[{now}] 缓存已保存，来源：{'强制刷新' if from_force else '自动更新'}")
    print(f"[{now}] 存档已保存：{archive_file}")
...
async def fetch_from_third_party():
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            print(f"[{datetime.now()}] 开始请求第三方 API: {THIRD_PARTY_API_URL}")
            ...
            print(f"[{datetime.now()}] 第三方 API 请求成功")
...
async def get_career_data(
    ...
    # 缓存无效，从第三方 API 获取
    try:
        print(f"[{datetime.now()}] 缓存无效，从第三方 API 获取数据")
        data = await fetch_from_third_party()
        # 保存到缓存
        ...
        # 如果第三方 API 失败，尝试返回过期缓存
        if cache_data:
            print(f"[{datetime.now()}] API 失败，返回过期缓存")
            return {
                "code": 200,
...
async def refresh_cache(
    ...
    参数：
    - force: 设置为 true 时强制从第三方 API 获取（忽略缓存）
    """
    try:
...
async def get_archive(date: str):
    ...
    """
    archive_file = DATA_DIR / f"cache.{date}.json"
    
    if not archive_file.exists():
        raise HTTPException(status_code=404, detail=f"日期 {date} 的存档不存在")
